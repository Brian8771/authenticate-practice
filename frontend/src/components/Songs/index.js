...
function Songs() {
    ...
    console.log(albums);
    useEffect(() => {
        dispatch(songActions.getSongs()).then(dispatch(getAllAlbums())).then(dispatch(songActions.getSongsByUser())).then(() =>
            setIsLoaded(true))
    }
        , [dispatch, songs])
    ...
    return (
        <div style={{ display: 'flex', justifyContent: 'center', backgroundColor: '#ECECEC', height: '100vh', width: '100%', flexDirection: 'column', alignItems: 'center' }}>
            ...
            <h2 className='header' style={{ backgroundColor: 'white', width: '80%', margin: 0, paddingTop: '30px', height: '10vh' }}>Albums</h2>
            <ul className='grid' >
                <div className="innerGrid">
                        ...
                        albums.map(song =>
                            <div style={{ display: 'flex', justifyContent: 'center', alignItems: 'center' }}>
                                ...
                                <li className='gridChildren' key={song.id}  >
                                    ...
                                    <br />
                                    <NavLink to={`/artists/${song.User.id}`} style={{ textDecoration: 'none', display: 'border-box', width: '160px', fontSize: '14px', margin: 0, color: '#c6c6c6' }}>
                                        {song.User.username}
                                    </NavLink>
                                    <div style={{ fontWeight: 550, fontSize: '14px' }}>
                                        {song.title}
